...
function init(){
	...
	texture = THREE.ImageUtils.loadTexture("uploads/" + LAST_FILE);
...
function screenshot(renderer) {
    if (event.keyCode == "32") {
        ...
        function grabScreen(renderer) {
            ...
            // var img = new Image();
            // img.src = file;
            var filename = CURRENT_DESIGNER + "_" + Date.now();
            // console.log(filename);
            var renderedImage = renderer.domElement.toDataURL('image/png');
